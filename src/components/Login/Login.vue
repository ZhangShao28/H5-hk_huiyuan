...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loginBtn () {
        var that = this,cid = cookie.get('cid');
        that.dis_no = true
                ...
                that.$dialog.toast({
                  ...
                  callback: () => {
                    ...
                    cookie.set('token', response.data.data.token, {
                      ...
                      if(response.data.data.cid_type == 2){
                          that.$router.push('/ApplyAdmissionList')
                      }else if(response.data.data.cid_type == 3){
                          that.$router.push('/MyInfo')
                      }else{
                          that.$router.push('/NewsList?cid=' + cookie.get('cid'))
                      }
                      cookie.set('cid_type', response.data.data.cid_type, {expires: 30000, path: '/'})
                  }
                })
              } else {
