<template>
    <div class="apply_admission">
        ...
        <div class="apply_head" v-if="cids==1 || cid_type>0">
            <group>
                <selector title="商會" v-model="value" :value-map="['cid', 'property_name']" :options="associationList"
                          @on-change="onChange" v-if="cid_type==0"></selector>
				<selector title="屬會" v-model="value" :value-map="['cid', 'property_name']" :options="associationList"
                          @on-change="onChange" v-if="cid_type>0"></selector>
            </group>
        </div>
        ...
        <div class="apply_con" :class="cids==1?'':'top40'">
            <div class="apply_no" v-if="apply_state==1">
                ...
                <div class="content">
                    <group>
                        ...
                        <div class="xin" v-if="cid_type==0"><span>*</span>
                            <x-input title="姓名英文" required v-model="username_en" class="pleft0" name="username_en"
                                     placeholder="請輸入姓名英文" type='text'></x-input>
                        </div>
                         <x-input v-else title="姓名英文" v-model="username_en" class="pleft0" name="username_en"
                                     placeholder="請輸入姓名英文" type='text'></x-input>
                        <div class="xin"><span>*</span>
                        ...
                        <div class="xin" v-if="cid_type==0"><span>*</span>
                            <selector title="登  記" v-model="voter" :options="voter_list"
                        ...
                        <div class="xin"><span>*</span>
                            <x-address @on-hide="logHide" raw-value value-text-align="left" :show.sync="showAddress"
                                       ...
                                       @on-shadow-change="onShadowChange" placeholder="請選擇"></x-address>
                        </div>
                        <div class="xin"><span>*</span>
                            <x-input title="詳細住址" v-model="address" required class="pleft0" name="address"
                                     placeholder="請輸入詳細住址" type='text'></x-input>
                        </div>
                        <div class="xin"><span>*</span>
                ...
                <div class="clause"  v-if="cid_type==1" style="text-align: left">
                    <check-icon :value.sync="selected_clause">本人已閱讀及同意以下條款</check-icon>
                    <span style="color: #3D82D9;"><router-link to="/Clausegx" style="text-align: center">《申請條款》</router-link></span>
                </div>
                <div class="submit_btn">
                    ...
                    <div class="back_btn" @click="back_btn" v-if="cid_type==0"><img src="/static/img/ic_return.png"
            ...
            <div class="apply_yes" v-if="apply_state==2">
                ...
                <div class="examine" v-if="status==2">
                    ...
                    <div class="examine_title">你已提交入會申請，請耐心等待</div>
                </div>
...
<script>
  import { XInput, Group, XButton, Cell, Toast, cookie, Selector, Datetime, XAddress, CheckIcon } from 'vux'
  ...
  export default {
    ...
    data () {
      return {
        ...
        address_phone: '',
        ID_number: '',
        ID_placeholder: '1位英文+4位數字',
        ID_length: 5,
        ID_mask: 'A9999',
        birth_date: '請選擇',
        sex: '0',
        sex_list: [{key: '0', value: '請選擇'},{key: '1', value: '男'}, {key: '2', value: '女'}],
        voter: '1',//是否選民
...
//        addressvalue: ['香港', '香港島', '中西區'],
    ...
    mounted () {
      ...
      var that = this
      var url_pay_state = this.$route.query.pid, url_cid = this.$route.query.cids != undefined ? this.$route.query.cids : this.$route.query.cid
      if (url_pay_state == 1) {
        that.$dialog.toast({
          mes: '支付成功！',
          timeout: 1500,
      ...
      that.url_cid = url_cid
        if(cookie.get('cid_type') != 0){
            that.cid = url_cid
            that.getAdmissionList(that.cid)
            that.getData(that.cid)
        }else if (cookie.get('cid') == 1) {
...
//            console.log(response.data.data)
            let list = response.data.data.property
      ...
      getData(cid){
          console.log('url_cid is '+this.url_cid)
          console.log('getData cid is '+cid)
        ...
        that.api.admissionIndex({
          ...
          .then(response => {
            ...
            const data = response.data
              that.cid_type = data.data.cid_type
            if (data.state == 1) {//未申請入會
              if (data.data.subscription_apply == 1) {
                ...
                that.ID_number = data.data.ID_number
               that.birth_date = data.data.birth_date ? data.data.birth_date : ''
...
//                    var status = 3 , is_charge = 2
      ...
      submit_btn(){
        ...
        if (that.username_en == '' && that.cid_type == 0) {
          that.$dialog.toast({
            mes: '姓名英文不能爲空！',
            timeout: 1500,
            icon: 'error'
        ...
        if (that.address == '') {
          ...
          if (that.selected_clause == false && that.cid_type == 1) {
              that.$dialog.toast({
                  mes: '請勾選申請條款！',
                  timeout: 1500,
                  icon: 'error'
        ...
        this.loading_show = true
          var sub_cid = that.url_cid != 0 ? that.url_cid : that.cid
