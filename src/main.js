...
import Clausegx from './components/ApplyAdmission/clauseGX'
import wenjuanDetail from './components/wenjuan/wenjuanDetail'
import wenjuanList from './components/wenjuan/wenjuanList'
Vue.use(VueRouter)
Vue.use(YDUI)
...
const routes = [
  ...
  {
    path: '/wenjuanDetail',//修改手機號
    name: 'wenjuanDetail',
    component: wenjuanDetail,
    meta: {
      keepAlive: false,
      requireAuth: false,
      keepAliveList: false
  ...
  {
    path: '/wenjuanList',//修改手機號
    name: 'wenjuanList',
    component: wenjuanList,
    meta: {
      keepAlive: true,
      requireAuth: true,
      keepAliveList: true
  ...
  if (to.meta.requireAuth) {
    ...
    if (t_state == -1) {//未登录
      ...
      if (c_state == -1) {//未授权
        ...
        // console.log('已授权已登录')
        if(to_path=='login'){
            var cid_type = cookie.get('cid_type');
            if(cid_type == 2){
                next('/ApplyAdmissionList')
            }else if(cid_type == 3){
                next('/MyInfo')
            }else{
